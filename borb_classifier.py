...
class BORB():
    def __init__(self, max_sample_size, th, l0, l1, m, n_updates):
        self.base_learner = create_nb_model()
    ...
    def train(self, df_train, **kwargs):
        ...
        for i in range(self.n_iterations): #linha 11
            ...
            if df_train.shape[0] > 0:
                ...
                df_output = self.base_learner.predict(X_df_ma.values) # 𝑖𝑟1 ← 1/𝑤 ∑︀𝑡 𝑗=𝑡−𝑤+1 𝑃𝑟𝑒𝑑𝑖𝑐𝑡(𝐵, ⃗𝑥𝑗, 0.5); (linha 14)

                # df_output_prediction = df_output['prediction'];
                self.ma = df_output.mean()
    ...
    def predict(self, df_features, **kwargs):
        return self.__track(self.base_learner.predict(df_features, **kwargs))

    def predict_proba(self, df_features, **kwargs):
        return self.__track(self.base_learner.predict_proba(df_features, **kwargs))
